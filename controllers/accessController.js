  ...
  checkIn: async (req, res) => {
    try {
      ...
      if (!user || user.is_active === false) {
        return res.status(403).json({ msg: "Usuario desactivado, acceso denegado" });
      }
      ...
      if (!sensor || !sensor.is_active) {
        return res.status(403).json({ msg: "Sensor desactivado, acceso denegado" });
      }
      if (!sensor.classrooms || sensor.classrooms.is_blocked) {
        return res.status(403).json({ msg: "Salón bloqueado, acceso denegado" });
      }
